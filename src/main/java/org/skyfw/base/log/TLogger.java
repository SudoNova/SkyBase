...
public final class TLogger {
    ...
    /**
     * <p>
     * Default log level for printing on the console, which is
     * {@link TMCodeSeverity#DEBUG}.
     * </p>
     */
    public static TMCodeSeverity defaultMinLogLevelForPrint = TMCodeSeverity.DEBUG;
    /**
     * <p>
     * Default log level for printing to the log file (if any), which is
     * {@link TMCodeSeverity#WARNING}.
     * </p>
     */
    ...
    /**
     * <p>
     * Actual log level for printing on the console, which is set to
    ...
    static {
        ...
        try {
            ...
            final String defaultLogPath = FilenameUtils.separatorsToSystem(TSystemUtils.getExecPath() + "\\Logs" + "\\"
                    + calendar.get(Calendar.YEAR) + "\\" + (calendar.get(Calendar.MONTH) + 1) // LocalDate.now().getMonthValue()
                    + "\\" + calendar.get(Calendar.DAY_OF_MONTH) + "\\" + calendar.get(Calendar.HOUR) + "-"
                    + calendar.get(Calendar.MINUTE) + "-" + calendar.get(Calendar.SECOND) + "___"
    ...
    private void init(String className) {
        ...
        /*
         * if (className != null) { this.className = className; Class clazz=
         * TSkyClassLoaderImpl1.loadByName(className); if (clazz != null){ fileName=
         * clazz. } } else
         */ {
     ...
     * of a function specifically. Debug - Information that is diagnostically
     * helpful to people more than just developers (IT, sysadmins, etc.). Info -
     ...
     * care about under normal circumstances. This is my out-of-the-box config
     * level. Warn - Anything that can potentially cause application oddities, but
     * for which I am automatically recovering. (Such as switching from a primary to
     * backup server, retrying an operation, missing secondary data, etc.) Error -
     * Any error which is fatal to the operation, but not the service or application
     * (can't open a required file, missing data, etc.). These errors will force
     * user (administrator, or direct user) intervention. These are usually reserved
     * (in my apps) for incorrect connection strings, missing services, etc. Fatal -
    ...
    public void traceBegin(String logMessage) {
        ...
        if (minLogLevelForPrint.getValue() > TMCodeSeverity.TRACE.getValue())
            return;
        this.doLog(TMCodeSeverity.TRACE, logMessage, null, -1, null, TLogRecord.TProgressStatusLogType.BEGINING,
                (byte) 0);
    }
    ...
    public void infoBegin(String logMessage) {
        ...
        if (minLogLevelForPrint.getValue() > TMCodeSeverity.INFO.getValue())
            return;
        this.doLog(TMCodeSeverity.INFO, logMessage, null, -1, null, TLogRecord.TProgressStatusLogType.BEGINING,
                (byte) 0);
    }
    ...
    // There are already heavily tuned loggers such as log4j. I believe writing
    // everything
    // from the scratch is the worst idea. At very least, some can extend log4j
    // components
    ...
    private synchronized void doLog(TMCodeSeverity logLevel, String logDescription, String methodName, int lineNumber,
        ...
        /* if ( e.getStackTrace().length > 4) */ {
            ...
            /*
             * stackTraceElements= Arrays.copyOfRange(e.getStackTrace(), 4,
        ...
        if ((stackTraceType.equals(TLoggerStackTraceType.ALWAYS_AUTO_STACK_TRACE))
                || (stackTraceType.equals(TLoggerStackTraceType.AUTO_STACK_TRACE_IF_NO_MANUAL_DATA_PROVIDED)
                        && !manualDataProvided)) {
    ...
    private StackTraceElement[] getCallStackFromIndex(int index) {
        ...
        /*
         * String nameofCurrMethod = new Throwable() .getStackTrace()[0]
         * .getMethodName();
         */
